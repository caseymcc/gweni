...
void printfVargs(std::string &_out, const char *_format, va_list _argList)
{
    ...
    char *out=temp;
    int len=vsnprintf(out, sizeof(temp), _format, _argList);
...
std::wstring widen(const gweni::String &nstr)
{
#ifndef AVOID_CPP11_CODECVT
...
gweni::String narrow(const std::wstring &wstr)
{
#ifndef AVOID_CPP11_CODECVT
    // wide to UTF-8 (C++11)
    ...
    gweni::String u8str=conv1.to_bytes(wstr);
...
void split(const gweni::String &str, const gweni::String &seperator,
    StringList &outbits, bool leave)
{
    size_t offset=0;
    ...
    {
        outbits.push_back(str.substr(offset, i-offset));
        offset=i+sepLen;
        i=str.find(seperator, offset);

        if(leave)
            offset-=sepLen;
    }

    outbits.push_back(str.substr(offset, length-offset));
}

int toInt(const gweni::String &str)
{
    if(str.empty())
        return 0;
...
float toFloat(const gweni::String &str)
{
    if(str.empty())
        return 0.0f;
...
bool toBool(const gweni::String &str)
{
    if(str.size() == 0)
        return false;
...
bool toFloats(const gweni::String &str, float *f, size_t count)
{
    StringList lst;
    split(str, " ", lst);
...
bool wildcard(const String &strWildcard, const String &strHaystack)
{
...
void toUpper(gweni::String &str)
{
    transform(str.begin(), str.end(), str.begin(), toupper);
}

void strip(gweni::String &str, const gweni::String &chars)
{
    gweni::String Source=str;
    str="";

    for(size_t i=0; i < Source.length(); i++)
    {
        if(chars.find(Source[i]) != gweni::String::npos)
            continue;
