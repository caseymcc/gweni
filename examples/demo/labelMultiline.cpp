...
LabelMultilineDemo::LabelMultilineDemo(controls::Base *parent, const String &name):
    Base(parent)
{
    dock(Position::Fill);
    controls::CrossSplitter *pSplitter=new controls::CrossSplitter(this);
    pSplitter->dock(Position::Fill);
    pSplitter->setMargin(Margin(10, 10, 10, 10));
    {
        controls::Label *label=new controls::Label(pSplitter);
